# ./python3 game.py <randomness> [loop, blue]
...
# Connecting to robot
robot_ip_address = "192.168.0.102"  # Replace by robot ip address, "10.10.10.10" in hotspot mode
...
# checks the input of the user for the menu
# TODO Adapt that wrong input is corrected
...
#default settings by starting game
    param = {
        "loop": 0,
        "turn_end": 2,
        "menu": 0,
    ...
    # loop off(0)/on(1), turn mode blue(0)/button(1)/learning_mode(2), menu off(0)/on(1), randomness, slope off(0)/on(1)
    try:
    ...
    for av in sys.argv[1:]:
        if av == "--menu" or av == "-m":
            ...
            param["loop"] = 1
        elif av == "--blue" or av == "-g":
            param["turn_end"] = 0  # blue shape
