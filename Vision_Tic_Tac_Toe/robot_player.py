...
class RobotPlayer:
    ...
    def robot_play(self, point, grid, slope):
        if slope:
            ...
            #self.__client.move_pose(*self.__slope_pos)
            #self.__slope_pos[2] = self.__slope_pos[2]-0.01
    ...
    def wait_other_player(self, turn_end_type, button_pin=PinID.GPIO_1A):
        ...
        if turn_end_type == 1:
            ...
            # wait for the blue token
            return self.wait_for_blue_token_move()
    ...
    def wait_for_blue_token_move(self):
        print("waiting blue token to be move...")
        blue_circle = None
        grid = None
        objects_list = []
        while not blue_circle:
            ...
            for obj_index, obj in enumerate(objects_list):
                if obj.color == ObjectColor.BLUE:
                    ...
                    break
        self.pick_place_on_grid(grid, blue_circle, -0.7, -0.7)
        return objects_list

    @staticmethod
    def no_more_circles_action():
        print("no more token in stock")
    ...
    def pick_place_on_grid(self, grid, obj, x, y, z=0):
        ...
        if not(abs(obj.obj_pos.x-(pos_min.x + size_x * (x + 0.5)))<=0.01 and abs(obj.obj_pos.y-(pos_min.y + size_y * (y + 0.5)))<=0.01):
