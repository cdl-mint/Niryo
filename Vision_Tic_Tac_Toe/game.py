...
class TicTacToe:
    #Initialisierung des Spiels mit den Eigenschaften
    ...
    def init_game(self):
        ...
        while len(self.__objects) > 0:
            ...
            #elif obj.color == ObjectColor.RED and l_ == 0 and self.__params["slope"] == 0:
            #    x, y, z = -0.7, -0.7, 0
            #    l_ = 1
    ...
    def play_game(self):
        ...
        while not end_game:  # loop for each turn
            ...
            #if robot should stop when cheating then the 3 lines below
            #end_game=self.cheat_detection(grid_o, self.__grid)
            #if end_game:
            #    return 
        ...
        for x in range(0, 3):
            for y in range(0, 3):
                if grid_1[x][y] != 0 and grid_2[x][y] == 0:
                    print("a token has been removed", x, y)
                    self.__robot.cheat_action()
                    return True
                elif grid_1[x][y] != 0 and grid_2[x][y] != grid_1[x][y]:
                    print("a token has been replaced", x, y)
                    self.__robot.cheat_action()
                    return True
                elif grid_1[x][y] == 0 and grid_2[x][y] == -1:
                    change += 1
                    if change > 1:
                        print("player has played multiple time")
                        self.__robot.cheat_action()
                        return True
            ...
            for y in range(0, 3):
                if len(grid[x][y + 1]) > 0:
                    if grid[x][y + 1][0].color == ObjectColor.GREEN:
                        game_grid[x][y] = 1
                    elif grid[x][y + 1][0].color == ObjectColor.RED:
                        game_grid[x][y] = -1
        return game_grid
